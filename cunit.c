...
processDir(char *cwd, List *okL)
{
	...
	dir = openDir(cwd);
	fprintf(stderr, "files were creat:\n");
	for(;;){
		file = readDir(dir);
		if(file == NULL){
			break;
...
compareOut(FILE *fdok, FILE *fdout, char *fok, char *fout)
{
	...
	for(;;){
		...
		if((cok == NULL) && (cout != NULL)){
			fprintf(stderr, "not pass test: %s - %s\n",fok,fout);
			statexit = 1;
			break;
...
checkOuts(List *okL)
{
	...
	if((*okL).first == NULL){
		fprintf(stderr, "there aren't test\n");
		return;
	}
	aux = (*okL).first;
	for(;;){
		...
		fdout = openFile(fout);
		if(fdout == NULL){
			fprintf(stderr, "it isn't out to test:%s\n",fok);
			aux = (*aux).next;
			continue;
		}
		if(fdok == NULL){
